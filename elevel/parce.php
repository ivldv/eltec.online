...
if ($sheet_xml->sheetData) {
    ...
    if ($r>=2) {
        $cell['producer'] = $end_result[4];
        $cell['artikle'] = substr ( $end_result[1] , 0 ,250 ); //артикул
        $cell['name'] = $end_result[0]; //наименование
        $cell['presence'] = $end_result[6];//количество
        $cell['additional'] = $end_result[7];
        $cell['cost'] = $end_result[9];
